...
React (Vite) ── POST /api/analyze ──► Express (Node.js)
                                        ...
                                        ├─ paralel Haiku çağrıları (Promise.all, structured output)
                                        └─ Sonnet sentez çağrısı
...
- **Modeller:** perspektifler `claude-haiku-4-5` (ucuz, paralel), sentez
  `claude-sonnet-5`. Çıktılar `output_config.format` (json_schema) ile garanti JSON.
